...
% \mathse{
%     \phi = \sinput{2_phi_e}\;(\text{don't care, just chose this}),\;\; \theta_e = \sinput{2_theta_e},\;\;\text{and}\;\;\psi_e = \sinput{2_psi_e}\;(\text{don't care, just chose this}). 
% }
% This results in the equilibrium state,
% \mathse{
%     x_e = \mat{\dot{p}_n\\ \dot{p}_e\\ \dot{p}_d\\ \dot{u}\\ \dot{v}\\ \dot{w}\\ \dot{e}_0\\ \dot{e}_1\\ \dot{e}_2\\ \dot{e}_3\\ \dot{p}\\ \dot{q}\\ \dot{r}} = \sinput{2_dot_x_e}.
% }
% Substituting,
% \begin{align*}
%     \mat{\dot{p}_n\\ \dot{p}_e\\ \dot{p}_d} &= \sinput{M1}\sinput{x1} + \sinput{B1_F}F + \sinput{B1_tau}\tau\\
